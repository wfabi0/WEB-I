...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome_form) || empty($descricao) || empty($preco) || empty($quantidade_estoque)) {
        ...
        if (editaProduto($id, $nome_form, $descricao, $preco, $quantidade_estoque)) {
        ...
        <div class="form-container">
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <input type="text" id="id" value="<?= htmlspecialchars($produto['id']) ?>" disabled>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="descricao" name="descricao" rows="4" required><?= htmlspecialchars($_POST['descricao'] ?? $produto['descricao']) ?></textarea>
                </div>

                <div class="form-group">
                    <label for="preco">Preço (R$)</label>
                    <input type="number" id="preco" name="preco" step="0.01" value="<?= htmlspecialchars($_POST['preco'] ?? $produto['preco']) ?>" required>
                </div>

                <div class="form-group">
                    <label for="quantidade_estoque">Quantidade em Estoque</label>
                    <input type="number" id="quantidade_estoque" name="quantidade_estoque" value="<?= htmlspecialchars($_POST['quantidade_estoque'] ?? $produto['quantidade_estoque']) ?>" required>
                </div>
